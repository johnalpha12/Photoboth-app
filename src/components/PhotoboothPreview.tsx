...
export default function PhotoboothPreview({ template, photos, onReset }: PhotoboothPreviewProps) {
  ...
  const drawCustom = (ctx: CanvasRenderingContext2D, img: HTMLImageElement, slotX: number, slotY: number, slotW: number, slotH: number, t: Transform) => {
    ...
    const renderRatio = renderBoxW / renderBoxH;
    let finalW, finalH, finalX, finalY;
    
    if (imgRatio > renderRatio) {
      finalH = renderBoxH;
      finalW = img.width * (renderBoxH / img.height);
      finalX = renderBoxX - (finalW - renderBoxW) / 2;
      finalY = renderBoxY;
    } else {
      finalW = renderBoxW;
      finalH = img.height * (renderBoxW / img.width);
      finalX = renderBoxX;
      finalY = renderBoxY - (finalH - renderBoxH) / 2;
    }
              ...
              >
                ...
                <img 
                  ...
                  style={{ 
                    ...
                    transform: `translate(${transforms[i].x}px, ${transforms[i].y}px) scale(${transforms[i].scale})`,
